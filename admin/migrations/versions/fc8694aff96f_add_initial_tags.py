...
def upgrade():
    ...
    _add_ingredients_to_tag(
        'top',
        list(set(spices + herbs + nuts_seeds + basics + fruit + vegetables + fish + seafood + meat + dairy))
    )
...
def _add_ingredients_to_tag(tag_slug: str, ingredient_slugs: List[str]) -> None:
    ...
    assert len(ingredient_ids) == len(ingredient_slugs), f"not all ingredients found {ingredient_slugs} ({len(ingredient_slugs)} != {len(ingredient_ids)}"
    op.execute(
        "INSERT INTO ingredient_tags (tag_id, ingredient_id) VALUES {}".format(
            ", ".join(f"({tag_id}, {ingredient_id})" for ingredient_id in ingredient_ids))
    )
    pass
...
def _get_ingredient_ids(slugs: List[str]) -> List[int]:
    ...
    if len(ingredients) != len(slugs):
        ...
        raise AssertionError(f"not all ingredients found: ({len(ingredients)} != {len(slugs)}) {missing}")
